...
void scene_model::setup_data(std::map<std::string, GLuint>& shaders, scene_structure&, gui_structure& gui)
{
    ...
    base_tree_cylinder = mesh_primitive_cylinder(0.05f, {0,0,0}, {0,-0.2f,0});
    base_tree_cylinder.uniform.color = vec3{153,51,0} / 255.0f;
    ...
    for (int k = 0; k < N_cone; ++k) {
      ...
      while (!good_pos) {
        ...
        vec3 new_pos = {x, -0.8f, z};
        good_pos = true;
        for (int j = 0; j < k; ++j) {
          if (vcl::norm(positions[j] - new_pos) < 0.3f)
            good_pos = false;
        }
        positions[k] = new_pos;
      }
...
void scene_model::frame_draw(std::map<std::string, GLuint>& shaders, scene_structure& scene, gui_structure&)
{
    ...
    for (int k = 0; k < N; ++k) {
      //float u = k / (N - 1);
      base_tree_cylinder.uniform.transform.translation = positions[k];
      draw(base_tree_cylinder, scene.camera, shaders["mesh"]);
    ...
    // Display cube
    // ********************************************* //
    cube.uniform.transform.rotation = rotation_from_axis_angle_mat3({ 0,0,1 }, std::sin(3 * time));
