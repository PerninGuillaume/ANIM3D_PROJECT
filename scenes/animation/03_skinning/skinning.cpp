...
buffer<joint_geometry> interpolate_skeleton_at_time(float time, const buffer< buffer<joint_geometry_time> >& animation)
{
    ...
    for(size_t k_joint=0; k_joint<N_joint; ++k_joint)
    {
        ...
        float t = time;
        float t1 = joint_anim[k_current].time;
        float t2 = joint_anim[k_current + 1].time;
        vec3 p1 = joint_anim[k_current].geometry.p;
        vec3 p2 = joint_anim[k_current + 1].geometry.p;

        const float alpha = (t-t1)/(t2-t1); //percentage of advancement in the current line, from 0 to 1
        const vec3 p = (1-alpha)*p1 + alpha*p2;

        const quaternion r = slerp(joint_anim[k_current].geometry.r, joint_anim[k_current + 1].geometry.r, alpha);
        const joint_geometry current_geometry = {p, r};
                      ...
                      const buffer<joint_geometry>& skeleton_rest_pose)
{
    ...
    for(size_t k=0; k<N_vertex; ++k)
    {
        ...
        // Array of all the influence of the different bones for one vertex
        vcl::buffer<skinning_influence> influences_vertex = skinning.influence[k];

        const size_t nb_bones = influences_vertex.size();
        mat4 M = mat4::zero();
        for (size_t j = 0; j < nb_bones; ++j) {
            skinning_influence influence_vertex = influences_vertex[j];
            float weight = influence_vertex.weight;
            int joint_nb = influence_vertex.joint;
            // Geometry of the joint at rest
            joint_geometry T_rest_geometry = skeleton_rest_pose[joint_nb];

            mat3 T_rest_rotation_mat = T_rest_geometry.r.matrix();
            // T_rest here to test
            mat4 T_rest = mat4::from_mat3_vec3(T_rest_rotation_mat, T_rest_geometry.p);
            mat4 T_rest_inverse = mat4::from_mat3_vec3(transpose(T_rest_rotation_mat)
                    , -1 * transpose(T_rest_rotation_mat) * T_rest_geometry.p);

            // Geometry of the joint at current position

            joint_geometry T_current_geometry = skeleton_current[joint_nb];

            mat3 T_current_rotation_mat = T_current_geometry.r.matrix();
            mat4 T_current = mat4::from_mat3_vec3(T_current_rotation_mat, T_current_geometry.p);

            M += weight * T_current * T_rest_inverse;
        }



        vec3 p = skinning.rest_pose[k];
        const vec4 rest_pos(p.x, p.y, p.z, 1.0f);
        vec4 tmp = M * rest_pos;
        p = vec3(tmp.x, tmp.y, tmp.z);
