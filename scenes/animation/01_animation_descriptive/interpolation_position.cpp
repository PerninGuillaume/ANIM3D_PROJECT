...
static vec3 cardinal_spline_interpolation(float t, float t0, float t1, float t2, float t3
...
void scene_model::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    if (idx == 0) {
      ...
      t0 = keyframes[idx - 1].t;
      p0 = keyframes[idx - 1].p;
    }

    float t3;
    vec3 p3;
    if (idx == N - 2) {
      t3 = keyframes[idx + 1].t;
      p3 = keyframes[idx + 1].p;
    } else {
      t3 = keyframes[idx + 2].t;
      p3 = keyframes[idx + 2].p;
    }
...
vec3 cardinal_spline_interpolation(float t, float t0, float t1, float t2, float t3
                                   , const vec3& p0, const vec3& p1, const vec3& p2, const vec3& p3) {
    const float tension = 1.0f;
    const vec3 d1 = tension * (p2 - p0) / (t2 - t0);
    const vec3 d2 = tension * (p3 - p1) / (t3 - t1);
    const float s = (t - t1) / (t2 - t1);
    vec3 p = (2 * std::pow(s, 3) - 3 * std::pow(s, 2) + 1) * p1
      + (std::pow(s, 3) - 2 * std::pow(s, 2) + s) * d1
      + (-2 * std::pow(s, 3) + 3 * std::pow(s, 2)) * p2
      + (std::pow(s, 3) - std::pow(s, 2)) * d2;
    return p;
}
